...
public class ExpertValueSourceEditorTests extends ComponentTest
    {
    @Test
    void displayAndChangePrimitiveValue()
        {
        ...
        Assertions.assertEquals(Long.toString(INT_VALUE), ((TextInputControl) text).getText());

        // change the value
        fillFieldAndTabAway(id(PrimitiveValueEditorField.INPUT_ID), Long.toString(INT_VALUE2));

        // check change to the underlying source
        Assertions.assertEquals(INT_VALUE2, source.getValue());
        }

    @Test
    void displayAndChangeType()
        {
        ...
        Assertions.assertEquals(_project.getValueSourceDescriptors().get(NotValueSource.TYPE_ID).getName(), ((MenuButton) chooser).getText());
    ...
    void showInitialSubsource()
        ...
        Assertions.assertEquals(quoted(varname), ((TextInputControl)subsource_field).getText());
        }

    @Test
    void addAndSetSubsource()
        {
        ValueSourceConfiguration source = ValueSourceConfiguration.forValue(INT_VALUE);
        _editor.setSource(source);
        waitForUiEvents();
        ...
        Assertions.assertNull(source.getSource());
        Assertions.assertFalse(exists(id(ExpertValueSourceEditor.SUBSOURCE_EDITOR_ID)));

        // add a subsource
        clickOn(id(ExpertValueSourceEditor.ADD_SUBSOURCE_ID));

        // subsource editor now present
        Assertions.assertTrue(exists(id(ExpertValueSourceEditor.SUBSOURCE_EDITOR_ID)));
        ...
        Assertions.assertEquals(123L, source.getSource().getValue());
        }

    @Test
    void removeSubsource()
        {
    ...
    void editAdvancedSubsource()
        ...
        Node chooser = lookup(id(ExpertValueSourceEditor.TYPE_FIELD_ID)).query();
        Assertions.assertEquals(_project.getValueSourceDescriptors().get(VariableValueSource.TYPE_ID).getName(), ((MenuButton) chooser).getText());
        ...
        Assertions.assertEquals(_project.getValueSourceDescriptors().get(StringValueSource.TYPE_ID).getName(), ((MenuButton) chooser).getText());

        // and the breadcrumb should be displayed
        Assertions.assertTrue(exists(ExpertValueSourceEditor.SUBSOURCE_BREADCRUMB_LABEL));
        ...
        Node subsource_field = lookup(id(ExpertValueSourceEditor.SUBSOURCE_EDITOR_ID)).query();
        Assertions.assertEquals(quoted("abc"), ((TextInputControl)subsource_field).getText());
        }

    @Test
    void editAdvancedNamedSource()
        {
        ...
        Node chooser = lookup(id(ExpertValueSourceEditor.TYPE_FIELD_ID)).query();
        Assertions.assertEquals(_project.getValueSourceDescriptors().get(LogMessage.TYPE_ID).getName(), ((MenuButton) chooser).getText());
        ...
        // and the breadcrumb should be displayed
        Assertions.assertTrue(exists(LogMessage.MESSAGE_PARAM));
        ...
        Assertions.assertEquals(newval, ((TextInputControl)subsource_field).getText());
        }

    @Test
    void editAdvancedIndexedSource()
        {
        ...
        // verify the displayed type is correct
        Node chooser = lookup(id(ExpertValueSourceEditor.TYPE_FIELD_ID)).query();
        Assertions.assertEquals(_project.getValueSourceDescriptors().get(AdditionSource.TYPE_ID).getName(), ((MenuButton) chooser).getText());
        ...
        // and the breadcrumb should be displayed
        Assertions.assertTrue(exists("[0]"));
        ...
        Assertions.assertEquals(quoted(newval), ((TextInputControl)subsource_field).getText());
        }
